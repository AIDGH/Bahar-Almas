...
- برای هر شماره زمان شروع پنجره، تعداد درخواست و زمان پایان قفل را نگه می‌دارد؛
- دو درخواست نخست هر پنجره‌ی ۶۰ ثانیه‌ای مجاز است و درخواست سوم قفل ۶۰ ثانیه‌ای می‌سازد؛
- به‌روزرسانی با یک Upsert اتمیک PostgreSQL انجام می‌شود تا درخواست‌های همزمان سقف را دور نزنند.
...
- Reward هنگام ثبت‌نام ساخته نمی‌شود؛ اولین Claim با امتیاز بیشتر از صفر آن را فعال می‌کند.
...
Migration اولیه در `20260904000000_init`، Migration بازی مهمان/پروفایل/دعوت در `20260908000000_guest_games_profiles_referrals`، Migration نقش ادمین/مسدودی در `20260909000000_admin_users_and_bans` و Migration محدودیت OTP در `20260909020000_otp_rate_limits` قرار دارند.

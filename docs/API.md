...
برای هر شماره دو درخواست در بازه‌ی ۶۰ ثانیه‌ای پذیرفته می‌شود. درخواست سوم پاسخ `429` با `retryAfterSeconds` می‌گیرد و تا پایان قفل یک‌دقیقه‌ای درخواست تازه‌ای ساخته نمی‌شود:
...
در `NODE_ENV=production` و `OTP_DELIVERY_MODE=preview`، درخواست ورود حساب دارای نقش `ADMIN` با `403` رد می‌شود. این محدودیت مانع تصاحب حساب ادمین از طریق کد نمایشی عمومی است و پس از اتصال Provider امن و خروج از حالت Preview اعمال نمی‌شود.
...
Challenge را مصرف می‌کند، در حالت ثبت‌نام User و کد معرف یکتا می‌سازد و Cookie را تنظیم می‌کند. ثبت‌نام شماره‌ی موجود دوباره داخل Transaction کنترل می‌شود تا Challenge قدیمی یا درخواست همزمان نتواند User تکراری بسازد. کد معرف در این مرحله فقط ارتباط دو User را ثبت می‌کند و هنوز امتیازی نمی‌دهد.
...
نیازمند Session ورود و Header `X-Game-Token` است. یک بازی `COMPLETED` و ثبت‌نشده را فقط یک‌بار به User فعلی متصل می‌کند، `totalGameScore` و در صورت لزوم `bestScore` را به‌روزرسانی می‌کند و Rank را برمی‌گرداند. اگر این اولین بازی امتیازدار User معرفی‌شده باشد، `ReferralReward` یکتا ساخته و ۱٬۰۰۰ امتیاز به معرف فعال اضافه می‌شود؛ Claimهای بعدی Reward را تکرار نمی‌کنند.
...
### `GET /admin/users?group=users&search=&limit=25&offset=0`

`group` یکی از `admins`، `users` یا `banned` است و پیش‌فرض آن `users` است. فیلتر گروه در سرور و پیش از صفحه‌بندی اعمال می‌شود. هر بخش با `bestScore DESC` مرتب است و `search` در همان بخش روی نام، شماره موبایل و کد معرف جست‌وجو می‌کند. پاسخ صفحه‌بندی‌شده شامل نقش، وضعیت مسدودی، رکورد، مجموع امتیاز، تعداد بازی‌ها و تعداد دعوت‌هاست.
...
هر فیلد اختیاری است، ولی حداقل یک تغییر باید ارسال شود. شماره جدید نرمال‌سازی و یکتا بررسی می‌شود. مسدودکردن، همه‌ی Sessionهای کاربر را همان لحظه حذف می‌کند؛ حساب و تاریخچه نگه داشته می‌شوند ولی ورود و حضور در لیدربورد متوقف می‌شود. ارسال `isBanned=false` رفع مسدودی را انجام می‌دهد. حساب دارای نقش `ADMIN` از این مسیر قابل مسدودکردن نیست. هیچ مسیر API برای تغییر `role` وجود ندارد و ارسال این فیلد با Validation سراسری رد می‌شود.

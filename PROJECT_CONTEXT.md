...
7. ادمین بتواند کاربران را به‌ترتیب رکورد جست‌وجو، ویرایش، مسدود یا رفع مسدودی کند.

## ۲. وضعیت فعلی

وضعیت: **Core Gameplay، بازی پیش از ورود، پروفایل، دعوت، لیدربورد صفحه‌بندی‌شده و مدیریت کاربران پیاده‌سازی شده و Build/Test را گذرانده است؛ نسخه‌ی قبلی روی ابرک ابرآروان با HTTP موقت فعال است و انتشار نسخه‌ی ادمین در حال انجام است**.
...
- کارت نتیجه‌ی تمام‌صفحه و موبایل‌محور که تا پایان Tap روی فضای بیرون، Scroll و تعامل صفحه‌ی پشت را قفل می‌کند؛
- ثبت نتیجه فقط پس از اعتبارسنجی سمت سرور؛
- منوی پروفایل کشویی و واضح در Header با شماره، نام تکرارپذیر و قابل ویرایش، چهار آمار امتیاز، کد/لینک دعوت و تاریخچه‌ی کامل شمسی بازی‌ها؛
...
- نقش‌های `USER` و `ADMIN` با Guard مستقل سمت سرور؛
- پنل مدیریت واکنش‌گرا برای فهرست مرتب‌شده بر اساس High Score، جست‌وجوی نام/شماره/کد معرف و ویرایش نام و شماره؛
- مسدودسازی نرم کاربر با ابطال فوری Sessionها، جلوگیری از ورود یا ثبت‌نام همان شماره و حذف از لیدربورد، بدون پاک‌شدن حساب یا تاریخچه؛
...
ثبت‌نام برای شماره‌ی موجود و ورود برای شماره‌ی ثبت‌نشده رد می‌شود. نام و نام خانوادگی یکتا نیست و از پروفایل یا پنل ادمین قابل تغییر است. کد معرف اختیاری فقط هنگام ثبت‌نام مصرف می‌شود. شماره‌ی مسدودشده نه می‌تواند وارد شود و نه حساب تازه بسازد.

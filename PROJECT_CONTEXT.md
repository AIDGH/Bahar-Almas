...
آخرین به‌روزرسانی محتوایی: ۲۰۲۶-۰۹-۱۰
...
5. کد معرف یکتا داشته باشد و پس از اولین بازی امتیازدار فرد معرفی‌شده امتیاز بگیرد؛
...
وضعیت: **Core Gameplay، بازی پیش از ورود، پروفایل، دعوت، لیدربورد صفحه‌بندی‌شده و مدیریت کاربران پیاده‌سازی شده، Build/Test را گذرانده و روی ابرک ابرآروان با HTTP موقت فعال است**.
...
- امکان دو درخواست OTP در هر بازه‌ی ۶۰ ثانیه‌ای و قفل یک‌دقیقه‌ای درخواست سوم، همراه شمارش معکوس در رابط؛
- مسدودبودن ورود تازه‌ی ادمین در Production تا وقتی OTP به‌صورت ناامن داخل صفحه Preview می‌شود؛ نشست معتبر فعلی ادمین حفظ می‌شود؛
- پیام‌های اعتبارسنجی تفکیک‌شده برای تعداد رقم، پیش‌شماره و کاراکتر نامعتبر شماره موبایل؛
...
- نمایش ترتیبی و دورتراز Targetِ «خرچ» و «خروچ» با WebPهای ثابت قبلی، انیمیشن نرم CSS، Cooldown مشترک ۹۰۰ میلی‌ثانیه‌ای و تأخیر ۴۲۰ میلی‌ثانیه‌ای بین دو نوشته؛ هر دو فایل پیش از شروع بازی دانلود و Decode و در Runtime مستقیماً از Asset ثابت رندر می‌شوند تا روی اتصال موبایل از دست نروند؛
...
- کارت نتیجه‌ی Portalشده در بالاترین لایه‌ی صفحه که لیدربورد را کامل تیره می‌کند و تا پایان Tap روی فضای بیرون، Scroll و تعامل صفحه‌ی پشت را قفل نگه می‌دارد؛
- ثبت نتیجه فقط پس از اعتبارسنجی سمت سرور؛
- منوی پروفایل مرکزی و واضح در Header با شماره، نام تکرارپذیر و قابل ویرایش، چهار آمار امتیاز، کد معرف و تاریخچه‌ی کامل شمسی بازی‌ها؛ صفحه‌ی نخست پروفایل پس از احراز نشست در پس‌زمینه Prefetch و درخواست همزمان آن Deduplicate می‌شود؛
- پنل پروفایل فشرده و کاملاً داخل کادر موبایل، همراه Viewport صریح، کنترل Text Autosizing و ورودی سازگار با Safari/WebView؛
- کد معرف یکتا با کپی مستقیم خود کد و پاداش تراکنشی ۱٬۰۰۰ امتیازی فقط پس از اولین بازی امتیازدار فرد معرفی‌شده؛
- رد ثبت‌نام مجدد شماره‌ی موجود هم هنگام درخواست کد و هم هنگام تأیید Challenge قدیمی؛
...
- پنل مدیریت واکنش‌گرا با سه بخش مستقل ادمین‌ها، کاربران عادی و مسدودشده‌ها، مرتب‌سازی High Score، جست‌وجوی نام/شماره/کد معرف، ویرایش نام و شماره و Ban/Unban؛
- جلوگیری چندلایه از ارتقای نقش: ثبت‌نام همیشه صریحاً `USER` می‌سازد، هیچ DTO عمومی یا مدیریتی فیلد نقش ندارد، ورودی اضافه رد می‌شود و مجوز ادمین در هر درخواست از دیتابیس خوانده می‌شود؛
...
- Cache مرورگر یک‌روزه همراه `stale-while-revalidate` هفت‌روزه برای Assetهای ثابت مسیر بازی؛
...
- ۳۴ تست واحد موفق برای Schedule نسخه‌بندی‌شده، Burst و فاصله‌ی بازیابی، سرعت/زاویه‌ی پرتاب، Combo scoring، افزایش سختی، Hit validation، تنظیم امن Cookie، خطاهای شماره موبایل، محدودیت OTP، ثبت‌نام تکراری، جایزه‌ی معرف پس از بازی، گروه‌بندی پنل، جلوگیری از تزریق نقش، حفاظت ورود ادمین، نشست کاربر مسدود و Guard ادمین؛
...
- انتشار موفق دسته‌بندی پنل مدیریت و تأیید روی Production: ورود تازه‌ی ادمین در Preview برابر `403`، تزریق نقش در ثبت‌نام برابر `400` و دسترسی بدون نشست به API مدیریت برابر `401` است؛
- انتشار موفق اصلاح بارگذاری موبایل روی Production و تأیید Healthy بودن Web/API و هدر Cache یک‌روزه‌ی فایل خرچ از مسیر عمومی Caddy؛
...
برای هر شماره دو درخواست کد در یک بازه‌ی ۶۰ ثانیه‌ای آزاد است. درخواست سوم یک قفل ۶۰ ثانیه‌ای ایجاد می‌کند و API زمان باقی‌مانده را برای شمارش معکوس رابط برمی‌گرداند.

در Production فعلی که کد OTP برای QA داخل Browser نمایش داده می‌شود، درخواست و Verify ورود ادمین مسدود است تا دانستن شماره به تصاحب حساب مدیریتی منجر نشود. حساب‌های عادی همچنان قابل تست‌اند و نشست ادمین موجود تا انقضا معتبر می‌ماند. پس از اتصال نجوا این قفل با حذف حالت `preview` خودکار کنار می‌رود.
